...
export function SelectionHandles({ bounds: b, rotation, rotatable, width, height, start, edit }: { bounds: Box; rotation: number; rotatable: boolean; width: number; height: number; start(e: PointerEvent<HTMLElement>, handle: DragHandle): void; edit(): void }) {
  const degrees = Math.round(normalizeAngle(rotation) * 180 / Math.PI) % 360;
