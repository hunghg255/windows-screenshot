...
- **Rotate:** select an arrow, rectangle/square, Text or Emoji, then drag the round handle above its frame to rotate freely through 360° in either direction. The angle is shown above the handle; hold Shift to snap to 15° steps. The eight resize handles follow the rotated axes. Text editing and size changes retain the angle. Shapes can rotate past an image edge; preview and output clip the same area.
- Rectangles/squares retain an even, fixed stroke width when resized; select one and use Size to change its outline thickness. Other drawing tools retain their existing stroke scaling.
