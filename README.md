...
- **Ctrl+Alt+F:** capture all connected displays, including taskbars, as one image.
- **Ctrl+Alt+R:** drag one region across connected displays. Release on any screen to open the editor; Escape cancels all overlays.
- Home and tray use the same whole-desktop capture as hotkeys, without a display chooser. Screens are arranged as in Windows, including negative positions and vertical layouts. Gaps remain transparent. Mixed-DPI screens use the highest connected scale factor as the output density; lower-DPI screens are resampled. Maximum: 32 megapixels for the desktop and total source frames, 16384 pixels per output edge; larger configurations report an error before capture.
...
- Draw straight lines, arrows, rectangles, circles, freehand strokes or blur. Hold Shift for a square. Color and size affect the next stroke; color also updates a selected colored object.
...
The editor header uses two tool rows without horizontal scrolling: drawing tools above, object properties and image actions below. Copy/Save/Cancel stay at the top right as icon buttons with tooltips. Successful Copy/Save messages appear in bold green at the bottom right.
